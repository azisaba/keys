...
parallel -C ' ' -a mapping.txt '{4} && ln -s /opt/resourcepacks {2}/resourcepacks || true'
parallel -C ' ' -a mapping.txt '{4} && ln -s /opt/minecraft/lgw/plugins {2}/lgwplugins || true'
parallel -C ' ' -a mapping.txt '{4} && ln -s /opt/minecraft {2}/server || true'
